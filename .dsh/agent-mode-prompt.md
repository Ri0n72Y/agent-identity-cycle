...
Shelves is an archival destination. New Shelves content must first exist as a Desktop working file. When the user explicitly asks to archive it, the Worker may move or organize that Desktop result into Shelves. After archival, remove the Desktop working copy when it is no longer active; keep a Desktop copy only when it still serves as a live editing entry point. Only an explicit request to revise an already-existing Shelves document permits direct modification of that old document without Desktop staging. Content looking stable is not itself archive authorization.
...
Manual Reflection also performs Short cleanup according to the configured retention policy: completed done slots older than the retention horizon may be removed when inactive, while retained legacy done slots with obsolete long bodies are normalized to one-line done stubs without being reflected again. If no retention horizon is configured, do not invent one.
