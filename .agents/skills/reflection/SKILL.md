...
Reflection is the slower consolidation stage of the persistent assistant. It reads recent entries from `<ASSISTANT_HOME>/short-memory.md`, decides what has become durable enough to affect longer-lived structures, and marks the Short entries it has considered without removing them from recent continuity.

The assistant has agency over when to invoke this skill. After Short Memory Appending has written a new `reflection:pending` entry, the assistant may trigger Reflection immediately when the interaction contains a meaningful project-state transition, an important correction, a durable user preference, a stable factual or tool discovery, a self-related change, or procedural learning that should not wait. If the new entry does not need immediate consolidation, leave it pending and continue working; a later user request, a future interaction, or a periodic scheduler can reflect over several pending entries together. Do not add a second per-turn obligation that forces Reflection to run after every Short write.

When the Harness supports subagents, prefer running Reflection in a dedicated subagent so the parent Agent does not fill its active context with historical files and consolidation details. Give the subagent the configured absolute assistant-home path, the pending Short entries or time range to consider, and the current task context needed to interpret them. The subagent should load only the target files needed by the relevant route, make the updates, and return a concise summary of what changed and any ambiguity that still requires parent judgment. If subagents are unavailable, run the same process directly.
...
Start from pending Short entries. Project state is the fastest slower-layer update: when an entry changes the current stage, latest result, active direction, or recovery point of a long-running project, update `memories/projects.md` directly from Short rather than waiting for Episode promotion. Use [references/projects.md](references/projects.md) for that document.

Then decide whether any part of the Short entry deserves durable factual preservation. Episode is the long-lived evidence layer and keeps `Self.md`, `User.md`, and `Facts.md`; archive only what future reasoning may need to reinterpret, using the shared memory format defined by the `short-memory-appending` skill. Use [references/episodes.md](references/episodes.md) for classification and content.
...
After all relevant routes for a Short entry have been considered, change its marker from `[reflection:pending]` to `[reflection:done]` even when the conclusion is that no slower file needs updating. `done` means the entry has been reviewed, not that it should be deleted; Short retention continues to follow its configured time window and activity rules. A later Reflection may still consult a `done` entry as nearby context while it remains in Short.
