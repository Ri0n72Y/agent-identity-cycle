...
新 Shelves 内容必须先形成 Desktop 工作稿。之后只有在用户明确要求归档时，才把该 Desktop 结果整理进入 Shelves；这两个动作可以在同一个明确的归档任务中连续完成，但不能省略 Desktop staging。

归档完成后，如果该工作稿已经不再承担后续活跃修改，默认将它移出 Desktop，不保留一份重复副本。只有在用户明确还要继续迭代、或同一稿件仍同时承担活跃工作入口时，才保留 Desktop working copy。Desktop 的目标是当前工作面，不是归档副本区。
